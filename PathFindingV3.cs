...
public class PathFindingV3 : MonoBehaviour
{
    Grid grid;
    ...
    private void Awake()
    {
        fSM_Wolf = Wolf.GetComponent<FSM_wolf>();
    ...
    {
        grid = GetComponent<Grid>();
        if (fSM_Wolf.curState_I == FSM_wolf.FSMState_I.Chasing)
        {
            if (wolf.targetSheep != null)
                Target = wolf.targetSheep.transform;
        }
        if (fSM_Wolf.curState_I == FSM_wolf.FSMState_I.Patorlling)
        {
            if (wolf.targetCube != null)
                Target = wolf.targetCube.transform;
        }

        timeToNextPathUpdate -= Time.deltaTime;
        if (timeToNextPathUpdate < 0)
        {
            ...
            PathFinder(Wolf.position, targetPosition);
